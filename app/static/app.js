...
const alignmentSelect = processForm.querySelector("select[name='alignment']");
const voiceStyleSelect = processForm.querySelector("select[name='voice_style']");
const voiceGenderSelect = processForm.querySelector("select[name='voice_gender']");

const ffmpegCommand = document.getElementById("ffmpegCommand");
const librarySelect = document.getElementById("librarySelect");
const libraryList = document.getElementById("libraryList");
const backgroundInput = processForm.querySelector(
	"input[name='background_video']",
);
const scriptExcelInput = processForm.querySelector(
	"input[name='script_excel']",
);
...
processForm.addEventListener("submit", async (event) => {
	...
	formData.append("text_color", processForm.text_color.value);
	formData.append("bg_color", processForm.bg_color.value);
	formData.append("bold", processForm.bold.checked ? "true" : "false");
	formData.append("italic", processForm.italic.checked ? "true" : "false");
	formData.append("alignment", alignmentSelect.value);
	formData.append("voice_style", voiceStyleSelect.value);
...
async function loadLibrary() {
	...
	payload.videos.forEach((video) => {
		...
		option.textContent = `${video.code} â€” ${video.title || "Untitled"}`;
		librarySelect.appendChild(option);

		const listItem = document.createElement("li");
		listItem.textContent = `${video.code} â€” ${video.title || "Untitled"}`;
